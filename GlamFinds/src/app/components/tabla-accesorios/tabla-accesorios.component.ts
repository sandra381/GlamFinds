...
// Tabla de administración de la categoría "Accesorios" (ruta "/tablaAccesorios").
// Lista todos los posts de accesorios en una tabla de Angular Material con
// filtro de texto, y permite borrar o ir a modificar cada fila.
@Component({
  selector: 'app-tabla-accesorios',
  templateUrl: './tabla-accesorios.component.html',
  styleUrls: ['./tabla-accesorios.component.scss']
})
export class TablaAccesoriosComponent {
  ...
  // Dispara la carga inicial de la lista de accesorios.
  ngOnInit(): void {

    this.listarAccesorios();
  }

  // Pide al backend todos los posts de la categoría "Accesorios" y los
  // carga en "dataSource.data" para que la tabla los muestre.
  listarAccesorios(){
    this.backend2.obtenerAccesorios().subscribe(x=>{
      console.log(x);
  ...
  // tabla navegando de nuevo a "/tablaAccesorios".
