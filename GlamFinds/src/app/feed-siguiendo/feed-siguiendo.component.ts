...
@Component({
  selector: 'app-feed-siguiendo',
  templateUrl: './feed-siguiendo.component.html',
  styleUrls: ['./feed-siguiendo.component.scss']
})
export class FeedSiguiendoComponent implements OnInit {
  ...
  cargarSiguiendo() {
    ...
    this.loading = true;
    this.backend1.obtenerFeedSiguiendo(userId).subscribe(
      async x => {
  ...
  private async inicializarDatos() {
    ...
    for (let i = 0; i < this.dataSource.length; i++) {
      ...
      promises.push(this.obtenerComentariosAsync(post.id_post));
      promises.push(this.countLikeAsync(post.id_post));
      promises.push(this.obtenerPerfilAsync(post.id_user));
    }
  ...
  async obtenerComentariosAsync(id_com: number) {
    return new Promise<void>(resolve => {
      this.backend1.obtenerComentarios(id_com).subscribe(z => {
  ...
  async obtenerPerfilAsync(id_us: number) {
    return new Promise<void>(resolve => {
      this.backend1.obtenerUsuario(id_us).subscribe(a => {
