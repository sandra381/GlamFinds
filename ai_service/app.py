...
async def analyze_outfit(request: dict):
    """
    Analiza una imagen y devuelve tanto las prendas segmentadas como los accesorios/joyería detectados.
    """
    image_path = request.get("image_path")
    if not image_path:
        raise HTTPException(status_code=400, detail="Se requiere 'image_path'")
    ...
    try:
        ...
        # ------------------ 2. Detección de joyería/accesorios ------------------
        CONFIDENCE_THRESHOLD = 0.05
        inputs_owl = owlvit_processor(text=[JEWELRY_LABELS], images=image, return_tensors="pt").to(device)

        with torch.no_grad():
            outputs_owl = owlvit_model(**inputs_owl)

        target_sizes = torch.tensor([[img_height, img_width]])
        results_owl = owlvit_processor.post_process_grounded_object_detection(
            outputs=outputs_owl,
            threshold=CONFIDENCE_THRESHOLD,
            target_sizes=target_sizes,
            text_labels=[JEWELRY_LABELS]
        ...
        jewelry_items = []
        for box, score, text_label in zip(results_owl["boxes"], results_owl["scores"], results_owl["text_labels"]):
            x_min, y_min, x_max, y_max = [int(v) for v in box.tolist()]
            x_min = max(0, x_min)
            y_min = max(0, y_min)
            x_max = min(img_width, x_max)
        ...
        # ------------------ 3. Análisis de maquillaje (si hay un rostro) ------------------
        makeup_zones = []
        face_detected = False
        skin_ref_rgb = None
        ...
        if MEDIAPIPE_AVAILABLE:
            try:
                results_face = face_mesh.process(img_array)  # ya está en RGB (viene de la imagen PIL)
                if results_face.multi_face_landmarks:
                    face_detected = True
                    landmarks = results_face.multi_face_landmarks[0]

                    skin_points = get_landmark_points(landmarks, SKIN_REF_INDICES, img_array_bgr.shape)
                    _, skin_cropped = get_roi_mask_and_crop(img_array_bgr, skin_points)
                    skin_colors = get_dominant_colors_from_array(skin_cropped)
                    skin_ref_rgb = skin_colors[0]
                    ...
                    for zone_name, indices in zones_config.items():
                        points = get_landmark_points(landmarks, indices, img_array_bgr.shape)
                        _, masked_crop = get_roi_mask_and_crop(img_array_bgr, points)
                        if masked_crop.size == 0 or np.sum(masked_crop) == 0:
                            continue
                        colors_zone = get_dominant_colors_from_array(masked_crop)
                        zone_color_rgb = colors_zone[0]
                        dist = compute_lab_distance(skin_ref_rgb, zone_color_rgb)
                        umbral = MAKEUP_THRESHOLDS.get(zone_name, 20.0)
                        has_makeup = dist > umbral
                ...
                # Una foto sin rostro (ej. flat lay de ropa) no debe tumbar todo el análisis
                print(f"Aviso: no se pudo analizar maquillaje en esta imagen: {e}")
                face_detected = False
                makeup_zones = []
                skin_ref_rgb = None
